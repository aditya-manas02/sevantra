...
import { Heart, Sprout, Sun, Leaf, Trophy, MapPin, ChevronRight, CheckCircle, Clock, CalendarHeart, Edit2, X, Phone, Info, Sparkles, Download } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const downloadImpactResume = () => {
    ...
    doc.text('Volunteer Impact Resume', 105, 20, { align: 'center' });
    
    doc.setTextColor(100, 100, 100);
    doc.setFontSize(12);
    doc.text('Generated by Sevantra - The Civic Engagement Platform', 105, 30, { align: 'center' });
    
    // Profile Info
    doc.setTextColor(40, 40, 40);
    doc.setFontSize(22);
    doc.setFont('helvetica', 'bold');
    doc.text(`${profile.firstName} ${profile.lastName}`, 20, 55);
    
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text(`Email: ${profile.email}`, 20, 65);
    if (profile.locationCity) doc.text(`Location: ${profile.locationCity}`, 20, 72);
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('IMPACT SUMMARY', 105, 95, { align: 'center' });
    doc.setFont('helvetica', 'normal');
    doc.text(`Total Hours: ${profile.stats?.totalHours || 0}`, 40, 105);
    doc.text(`Impact Points: ${profile.gamificationPoints || 0}`, 105, 105, { align: 'center' });
    doc.text(`Level: ${level.title}`, 170, 105, { align: 'right' });
    
    // Recent Events
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Recent Volunteer Events', 20, 135);
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    let yPos = 145;
    if (profile.recentEvents && profile.recentEvents.length > 0) {
      profile.recentEvents.forEach((event: any, i: number) => {
        if (i > 5) return; // Limit to top 6
        const dateStr = new Date(event.startDate).toLocaleDateString();
        doc.text(`• ${event.title} - ${dateStr} (${event.locationName})`, 25, yPos);
        yPos += 10;
      });
    } else {
      doc.text('No recent events recorded.', 25, yPos);
    }
    ...
    if (profile.badges && profile.badges.length > 0) {
      ...
      doc.setFontSize(16);
      doc.setFont('helvetica', 'bold');
      doc.text('Earned Badges & Achievements', 20, yPos);
    ...
    doc.text('This document serves as an official record of civic engagement and volunteer work.', 105, 280, { align: 'center' });
    
    doc.save(`${profile.firstName}_${profile.lastName}_Impact_Resume.pdf`);
  };
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-2">
            ...
            {currentUser?.id === profile.id && (
              <div className="flex items-center gap-2">
                <button 
                  onClick={downloadImpactResume}
                  className="flex items-center gap-2 px-4 py-2 bg-[var(--background)] hover:bg-[var(--secondary)] hover:text-white border border-[var(--border)] hover:border-[var(--secondary)] text-[var(--text-primary)] text-sm font-bold rounded-xl transition-all shadow-sm"
                >
                  <Download className="w-4 h-4" /> {t('profile.downloadResume', 'Impact Resume')}
                </button>
                <button 
                ...
                >
                  <Edit2 className="w-4 h-4" /> {t('profile.editProfile', 'Edit')}
                </button>
              </div>
            )}
          </div>
